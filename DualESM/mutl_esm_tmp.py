...
class DualEsmForClassification(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = self.esm_model_wt(**x_a).last_hidden_state
        x2 = self.esm_model_mutant(**x_b).last_hidden_state        
        x = torch.cat([x1, x2], 2)
...
if __name__ == "__main__":
    ...
    model1 = EsmModel.from_pretrained(model_name)
    model2 = EsmModel.from_pretrained(model_name)
...
with torch.no_grad():
    ...
    predictions = np.argmax(logits, axis=-1)
    print("genesplit")
    np.save("out/esm1b_genesplit/labels", labels)
    np.save("out/esm1b_genesplit/logigs", logits)
    np.save("out/esm1b_genesplit/predictions", predictions)
